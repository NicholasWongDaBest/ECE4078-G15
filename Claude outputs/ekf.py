...
class EKF:
    ...
    def __init__(self, robot):
        ...
        # Flat process-noise floor added to Q's robot-pose block on every
        # predict() call, on top of the encoder-tick-derived
        # robot.covariance_drive() term -- a hedge against real motion noise
        # that model doesn't capture (wheel slip, ground unevenness, timing
        # jitter). It's unconditional by nature, which is a problem during a
        # sustained in-place rotation: the robot's true x,y isn't changing,
        # but the full floor still gets added to P's x,y entries call after
        # call just from turning, producing an outsized corrective jump in
        # x,y the next time a landmark update lands. See rotation_fraction()
        # and predict(): the x,y part of this floor is scaled down in
        # proportion to how much of the current step is rotation rather than
        # translation (theta's share is never scaled -- heading uncertainty
        # should keep accumulating normally while turning).
        self.process_noise_floor = 0.01
        self.rotation_xy_noise_floor_scale = 0.1  # x,y floor at pure rotation = 10% of normal
