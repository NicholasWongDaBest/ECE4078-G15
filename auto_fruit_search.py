...
class FruitMapper:
    ...
    def observe(self, boxes, pose, only_labels=None, new_only=False):
        """Fuse one frame's (label, xywh box) list taken at `pose`. Returns
        how many sightings the filter accepted.

        only_labels: fuse ONLY sightings of these fruits (a targeted look at
            one weak fruit must not move the others). new_only: fuse only
            fruits never seen before -- lets a fruit first spotted mid-drive
            become an obstacle without letting a mid-drive pose move a fruit
            that is already on the map (even one seen only once: a second
            sighting with FruitEKF's 1e3 birth prior would replace it
            outright). Both are checked against the label the
            sighting is finally fused under, i.e. after the lemon/lime style
            relabel, not the detector's raw class."""
        ...
        for label, box in boxes:
            ...
            if alt is not None:
                x, y = alt
            if only_labels is not None and fuse_label not in only_labels:
                continue
            if new_only and fuse_label in self.fruit_ekf.estimates:
                continue
...
class Navigator:
    ...
    def __init__(self, botconnect, ekf, aruco_sensor, baseline,
        ...
        # Which fruit sightings the mapper may use right now (Level 3), see
        # fruit_mapping(): 'all' | 'new_only' | 'targets' | 'off'.
        self.fruit_map_mode = 'all'
        self.fruit_map_labels = None
        # Targeted look (look_at_fruit()): after checking its position, the
        # robot pans only +/- target_pan_half about the fruit it came to see,
        # in target_pan_step steps, instead of a full 360 deg scan.
        self.target_pan_half = np.deg2rad(25.0)
        self.target_pan_step = np.deg2rad(10.0)
        self.last_look_fused = 0
    ...
    def fruit_mapping(self, mode, labels=None):
        """
        Temporarily restrict which fruit sightings the Level 3 mapper takes:
          'all'      -- every fruit (the default outside exploration)
          'new_only' -- only fruits never seen before: a fruit first spotted
                        mid-drive still becomes an obstacle, but a pose that
                        is still being corrected cannot move one already mapped
          'targets'  -- only the fruits in `labels` (a targeted look)
          'off'      -- none: pose checks only
        Restores the previous setting on exit, even on an exception.
        """
        assert mode in ('all', 'new_only', 'targets', 'off'), mode
        previous = (self.fruit_map_mode, self.fruit_map_labels)
        self.fruit_map_mode = mode
        self.fruit_map_labels = set(labels) if labels is not None else None
        try:
            yield
        finally:
            self.fruit_map_mode, self.fruit_map_labels = previous

    def _map_filter(self):
        """observe() keyword arguments for the current fruit_map_mode, or None
        when mapping is off."""
        if self.fruit_map_mode == 'off':
            return None
        if self.fruit_map_mode == 'new_only':
            return {'new_only': True}
        if self.fruit_map_mode == 'targets':
            return {'only_labels': self.fruit_map_labels or set()}
        return {}
    ...
    def _map_fruits_from_last_frame(self):
        ...
        if self.fruit_mapper is not None and self._last_boxes and not self._suppress_mapping:
            flt = self._map_filter()
            if flt is None:
                return
            s = self.ekf.robot.state
            self.fruit_mapper.observe(self._last_boxes, (s[0, 0], s[1, 0], s[2, 0]), **flt)
    ...
    def scan_around(self, step=np.deg2rad(10.0), frames=None):
        ...
        for k in range(n_steps):
            ...
            status = "-"
            flt = self._map_filter()
            if boxes and self.fruit_mapper is not None and flt is None:
                status = "not mapped (fruit mapping off)"
            elif boxes and self.fruit_mapper is not None:
                if converged:
                    s = self.ekf.robot.state
                    accepted = self.fruit_mapper.observe(boxes, (s[0, 0], s[1, 0], s[2, 0]), **flt)
                    mapped_stops += 1
                    status = "mapped {}/{}{}".format(
                        accepted, len(boxes),
                        " (new fruits only)" if flt.get('new_only') else
                        " (only {})".format(", ".join(sorted(flt['only_labels'])) or "none") if 'only_labels' in flt else "")
                else:
                    skipped_stops += 1
    ...
    def look_at_fruit(self, targets, pan_half=None, step=None, frames=None):
        """
        Level 3 targeted look, run on arriving at a viewpoint that was chosen
        to see `targets` (usually one weak fruit) from a new bearing.
        Replaces the full 360 deg scan there, which re-observed every other
        fruit too -- from a heading nothing had confirmed yet.

          1. Check the position FIRST, with fruit mapping off, so nothing
             seen while the pose is being corrected moves any fruit. Skipped
             if the re-localisation that ended the last leg already converged.
          2. Turn towards the target's current estimate in small trusted
             steps (still reading markers, still mapping nothing), then pan
             slowly +/- pan_half about it (target_pan_half, 25 deg by default: the
             estimate's depth error from its earlier, narrow views shows up
             as a bearing error from here). Each stop dwells like
             scan_around(): markers keep correcting the pose on every frame,
             and the consensus boxes are fused only if the pose there has
             converged -- and ONLY for the targets. Other fruits that come
             into view are left exactly where they are. If the target was
             only in view at stops before the pose converged (the turn to
             face it costs heading certainty until a marker in the window
             restores it), the pan sweeps back once across the same window.
        @return: set of target labels seen during the pan
        """
        mapper = self.fruit_mapper
        targets = [t for t in targets if t]
        if mapper is None or not targets:
            return set()
        pan_half = self.target_pan_half if pan_half is None else float(pan_half)
        step = abs(self.target_pan_step if step is None else float(step))
        frames = self.scan_frames if frames is None else int(frames)
        names = ", ".join(targets)

        # 1. Where am I? Pose only -- no fruit is touched.
        self.display.notify(f"Checking position before looking at {names}")
        if self.relocalise_enabled and not (self.last_relocalise_converged and self.pose_converged()):
            print(f"  [look] checking position before looking at {names} (fruit mapping off)")
            with self.fruit_mapping('off'):
                self.relocalise()
        sd_x, sd_y, sd_t, _, _ = self.pose_uncertainty()
        s = self.ekf.robot.state
        print("  [look] position {}: [{:.3f}, {:.3f}, {:.1f}deg], sd {:.0f}/{:.0f} cm, {:.0f} deg".format(
            "confirmed" if self.pose_converged() else "NOT converged -- fruits will only be mapped at stops "
                                                       "where markers bring it in",
            s[0, 0], s[1, 0], math.degrees(_normalize_angle(float(s[2, 0]))),
            sd_x * 100, sd_y * 100, math.degrees(sd_t)))

        # 2. Pan about the target(s) only.
        est = mapper.positions()
        known = [t for t in targets if t in est]
        if not known:
            print(f"  [look] no estimate for {names} yet -- nothing to aim at")
            return set()
        x, y, theta = float(s[0, 0]), float(s[1, 0]), float(s[2, 0])
        bearings = [math.atan2(est[t][1] - y, est[t][0] - x) for t in known]
        centre = math.atan2(sum(math.sin(b) for b in bearings), sum(math.cos(b) for b in bearings))
        lo, hi = centre - pan_half, centre + pan_half
        if abs(_normalize_angle(lo - theta)) <= abs(_normalize_angle(hi - theta)):
            first, signed_step = lo, step
        else:
            first, signed_step = hi, -step
        n_steps = max(1, int(round(2 * pan_half / step)))
        dist = min(path_planner.dist_between((x, y), est[t]) for t in known)
        print(f"  [look] panning {math.degrees(pan_half):.0f} deg either side of {names} "
              f"(bearing {math.degrees(_normalize_angle(centre)):+.0f}deg, {dist:.2f} m away)")
        self.display.notify(f"Looking at {names}")

        # Turn to the window in the same small, slow, trusted steps the 360
        # deg scan uses, reading markers at every one (fruit mapping off).
        # One quick turn would be trusted to only 10%: a 90 deg turn alone
        # adds 9 deg of heading uncertainty, more than the 6 deg under which
        # a fruit may be fused, and a window with no marker in it could then
        # never be mapped at all.
        delta = _normalize_angle(first - theta)
        n_approach = int(math.ceil(abs(delta) / step - 1e-9))
        if n_approach:
            with self.fruit_mapping('off'):
        ...
        seen = set()
        fused = 0
        with self.fruit_mapping('targets', known):
            # Up to two sweeps across the window. Turning to face the fruit
            # costs heading certainty (a normal turn is trusted to 10%), so
            # the first stops can be unconverged until a marker in the window
            # re-pins the heading. If the fruit was only in view at stops
            # like that, sweep back: the pose is now converged, the pan steps
            # are small and trusted, and the fruit gets fused on the way back.
            for sweep in (1, 2):
                if sweep == 2:
                    signed_step = -signed_step
                    print(f"  [look] {names} was only in view before the pose converged -- sweeping back")
                missed = False
                for k in range(n_steps + 1):
                    if k > 0:
                        self.turn(signed_step, speed=self.pan_turn_speed, noise_frac=self.scan_turn_noise_frac)
                    elif sweep == 2:
                        continue   # already standing at this edge
                    dwell = []
                    n_ar_last = 0
                    self._suppress_mapping = True
                    try:
                    ...
                    boxes = self._consensus_boxes(dwell)
                    labels = [label for label, _ in boxes]
                    in_view = [l for l in labels if l in known]
                    seen.update(in_view)
                    status = "-"
                    if in_view:
                        if self.pose_converged():
                            s = self.ekf.robot.state
                            accepted = mapper.observe(boxes, (s[0, 0], s[1, 0], s[2, 0]), only_labels=set(known))
                            fused += accepted
                            status = "fused {} ({} accepted)".format(", ".join(in_view), accepted)
                        else:
                            missed = True
                            sd_x, sd_y, sd_t, _, _ = self.pose_uncertainty()
                            status = "NOT fused: pose not converged (sd {:.0f}/{:.0f} cm, {:.0f} deg)".format(
                                sd_x * 100, sd_y * 100, math.degrees(sd_t))
                    others = [l for l in labels if l not in known]
                    s = self.ekf.robot.state
                    print("  look {}.{:d}/{}: hdg {:+6.1f}deg, {} marker(s), target {} -> {}{}".format(
                        sweep, k + 1, n_steps + 1, math.degrees(_normalize_angle(float(s[2, 0]))), n_ar_last,
                        in_view or "-", status, "  (ignored: {})".format(", ".join(others)) if others else ""))
                if not (missed and self.pose_converged()):
                    break
        for t in known:
            p = mapper.positions().get(t)
            print("  [look] {}: now [{:+.2f}, {:+.2f}], {} view(s) over {:.0f} deg -- {}".format(
                t, p[0], p[1], mapper.fruit_ekf.n_views(t), mapper.fruit_ekf.bearing_spread(t),
                "well mapped" if mapper.well_mapped(t) else "still weak"))
        if not seen:
            print(f"  [look] {names} never came into view -- its estimate is unchanged")
        elif not fused:
            print(f"  [look] {names} was in view but the pose never converged at those stops -- "
                  f"nothing fused, estimate unchanged")
        self.last_look_fused = fused
        return seen
...
def run_level3(nav, search_list, aruco_true_pos, mapper, planner='astar', grid_resolution=0.05,
    ...
    different directions (FruitEKF.bearing_spread). There the robot does NOT
    scan 360 deg: it confirms its position first with fruit mapping off, then
    pans only about that fruit and fuses only that fruit (look_at_fruit()).
    Fruits never seen at all fall back to the fixed quadrant viewpoints,
    where a 360 deg scan is still needed to find them -- but only fruits not
    yet on the map are added there. Fruit mapping during exploration:
      centre scan               every fruit (at stops whose pose converged)
      driving between views     new fruits only (so they become obstacles)
      re-localising             none
      targeted look             the target only
      fixed-viewpoint scan      unmapped fruits + weak search-list fruits
    so the only frames that can move a fruit already on the map are the
    centre scan and a targeted look at that very fruit. The obstacle set
    grows as fruits are mapped, so exploration never plans through a fruit
    it has already seen, and a viewpoint that turns out to sit on a fruit is
    dropped. Stops once every search_list fruit is well mapped, or at
    max_viewpoints. The map is written to `map_out` in truemap.txt format.
    ...
    with nav.fruit_mapping('off'):
        pose = nav.get_robot_pose()
        pose = nav.settle_pose()   # re-localise only if the start pose isn't already pinned

    fixed = [tuple(v) for v in viewpoints]
    failed_looks = []   # viewpoints whose targeted look fused nothing: not offered again
    # A marker counts as 'in the look window' if it will be in frame at some
    # stop of the pan: pan half-width plus most of half the field of view
    # (not all of it: a marker right at the frame edge is often cut off).
    look_half = nav.target_pan_half + 0.8 * nav.camera_fov() / 2.0
    visited = 0
    while visited < max_viewpoints:
        ...
        target = None   # the fruit this viewpoint is for, when it is for one
        if visited == 0:
            ...
            if mapper.all_expected_mapped():
                ...
                break
            vp, why, target = _next_viewpoint(mapper, search_list, pose, obstacles, view_ring,
                                              markers=[tuple(m) for m in aruco_true_pos],
                                              look_half=look_half, avoid=failed_looks)
            if vp is None:
                if not fixed:
                ...
                vp, target = fixed.pop(0), None
                why = ("fixed viewpoint -- still unseen: " + ", ".join(unseen) if unseen else
                       "fixed viewpoint -- no targeted viewpoint left ({})".format(why))
            elif vp in fixed:
                fixed.remove(vp)

        visited += 1
        print(f"\n[explore] viewpoint {visited}: [{vp[0]:+.2f}, {vp[1]:+.2f}] -- {why}")
        display.notify(f"Exploring: viewpoint {visited}" + (f" for {target}" if target else ""))
        if path_planner.dist_between(vp, pose[:2]) > 0.10:
            # Driving there: a fruit seen for the first time still goes on the
            # map (it has to be an obstacle for the rest of the drive), but
            # nothing already mapped is moved by a mid-drive pose.
            with nav.fruit_mapping('new_only'):
        ...
        if visited == 1:
            # Centre scan: the one place every fruit is mapped at once.
            seen = nav.scan_around(step=scan_step)
            with nav.fruit_mapping('off'):
                pose = nav.settle_pose()
        elif target is not None:
            # Check the position first (no fruit moves), then pan about the
            # target only (only the target moves).
            seen = nav.look_at_fruit([target])
            if not nav.last_look_fused:
                failed_looks.append(tuple(vp))
            s = nav.ekf.robot.state
            pose = np.array([s[0, 0], s[1, 0], s[2, 0]])
        else:
            # No targeted viewpoint: a search-list fruit is still unseen (a
            # full scan is the only way to find it), or every look at a weak
            # one has failed. Check the position first, then scan -- fusing
            # only fruits with no real estimate and the weak search-list
            # fruits. Well-mapped fruits and mapped obstacles are left alone.
            with nav.fruit_mapping('off'):
                if nav.relocalise_enabled and not (nav.last_relocalise_converged and nav.pose_converged()):
                    nav.relocalise()
            est = mapper.positions()
            open_labels = {l for l in mapper.dims
                           if l not in est or mapper.fruit_ekf.n_shots.get(l, 0) <= 1
                           or (l in search_list and not mapper.well_mapped(l))}
            with nav.fruit_mapping('targets', open_labels):
                seen = nav.scan_around(step=scan_step)
            with nav.fruit_mapping('off'):
                pose = nav.settle_pose()
...
def _next_viewpoint(mapper, expected, pose, obstacles, ring, bounds=path_planner.ARENA_BOUNDS,
                    detect_range=1.2, markers=None, look_half=None, marker_bonus=0.6, avoid=(),
                    avoid_radius=0.15):
    """
    ...
    estimate on 12 bearings. A candidate scores how new a bearing it offers
    on the fruit it is placed around (only that fruit is looked at and fused
    there, see look_at_fruit()) -- capped at 90 deg, since a right-angle crossing already
    ...
    markers/look_half: a candidate also scores marker_bonus for each of up
    to two markers (1-2 m away) that will be in frame while the robot pans
    +/- look_half about the fruit. The look only fuses the fruit at stops
    where the pose has converged, and turning to face the fruit costs
    heading certainty that only a marker IN THAT WINDOW can win back -- a
    viewpoint with nothing behind the fruit sees it and can't use it.
    avoid: viewpoints whose look already failed; nothing within avoid_radius
    of one is offered again.
    @return: ((x, y), reason, target) -- target being the weak fruit the
        viewpoint was placed around, the one look_at_fruit() then pans about
    ...
    for label in weak:
        ...
        for k in range(12):
            ...
            if path_planner.point_in_collision(p, obstacles):
                continue
            if any(path_planner.dist_between(p, a) < avoid_radius for a in avoid):
                continue
            # Only the fruit this candidate is placed around counts: the
            # look there pans about that fruit and fuses nothing else, so a
            # new bearing it would offer on some other fruit is never used.
            score = (gain_for(label, p)
                     - 0.4 * path_planner.dist_between(p, pose[:2])
                     - 0.8 * max(0.0, math.hypot(p[0], p[1]) - 0.6 * half))
            if markers is not None and look_half is not None:
                to_fruit = math.atan2(fy - p[1], fx - p[0])
                n_window = sum(1 for mx, my in markers
                               if 0.35 <= math.hypot(mx - p[0], my - p[1]) <= 1.8
                               and abs(_normalize_angle(math.atan2(my - p[1], mx - p[0]) - to_fruit)) <= look_half)
                score += marker_bonus * min(n_window, 2)
            if score > best_score:
                best, best_score, best_label = p, score, label
                best_why = "new bearing on {} (seen over only {:.0f} deg so far)".format(
                    label, mapper.fruit_ekf.bearing_spread(label))
    if best is None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--target-pan", type=float, default=25.0,
                         help="Level 3: half-width of the pan about the fruit a viewpoint was chosen for, deg "
                              "(replaces the 360 deg scan there)")
    ...
    nav.target_pan_half = np.deg2rad(args.target_pan)
