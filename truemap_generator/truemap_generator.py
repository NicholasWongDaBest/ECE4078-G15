...
class MainView(QMainWindow):
    ...
    def SaveBtnClicked(self):
        EstimateMap = {}
        for ArUco in self.EstimatedArUcos:
            EstimateMap[f"aruco{ArUco[0]}_0"] = {"x": ArUco[1][0], "y": -ArUco[1][1]}

        for Object in self.EstimatedObjects:
            EstimateMap[f"{self._params.Objects[Object[2]][0]}_{Object[0]}"] = {"x": Object[1][0], "y": -Object[1][1]}

        with open(self._params.SaveImageName, "w") as file:
            json.dump(EstimateMap, file, indent=4)
            print("Saved map!")
    ...
    def loadParams(self):
        ...
        colour_code = [tuple(reversed(ast.literal_eval(row['rgb display']))) for row in data] # reversed to be BGR
        
        objects = []
        for i in range(len(object_list)):
            objects.append([object_list[i], colour_code[i]])

        self._params = Parameters(
            DesiredGroundTruth=cwd + '/' + params['reference_map'],
